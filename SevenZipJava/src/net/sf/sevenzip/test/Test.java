...
public class Test {
	...
	private static String cryptedTestFilenames[] = { "TestArchives//TestContent-crypted.7z",
		...
		try {
			IInArchive archive = SevenZip.openInArchive(
					ArchiveFormat.SEVEN_ZIP, new InStreamImpl(
							new RandomAccessFile(new File(cryptedTestFilenames[0]),
									"r")), new TestArchiveOpenCallback());
