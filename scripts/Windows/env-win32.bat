...
REM ???? SET PATH=%PATH%;%MINGW_HOME%/bin

SET CMAKE_TOOLCHAIN=-G"MinGW Makefiles" -DMINGW32=Yes -DCMAKE_RC_COMPILER:FILEPATH=%CMD_WINDRES%  -DCMAKE_C_COMPILER:FILEPATH=%CMD_GCC% -DCMAKE_CXX_COMPILER:FILEPATH=%CMD_GPP% -DCMAKE_SHARED_LINKER_FLAGS:STRING="-specs %MY_DIR%\gcc64.specs" -DRUNTIME_LIB=%RUNTIME_LIB%
