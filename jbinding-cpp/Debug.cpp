...
void TracePrintObjectsUsingPrintf() {
	...
	int count = 1;
	for (; i != g_classes_map.end(); i++) {
		printf("> %3i %s (this: 0x%08X)\n", count++, (*i).second->_classname,
				(size_t)(*i).second->_thiz);
	}
	fflush(stdout);
...
void TraceObjectDestruction(void * thiz) {
	...
	if (found) {
		fatal(
				"TraceObjectDestruction(): destructor called for unknown this=0x%08X",
				(size_t) thiz);
	}
...
void TraceObjectCall(void * thiz, const char * methodname) {
	...
	if (found) {
		fatal("Object call for dead object. Method name: %s, this: 0x%08X",
				methodname, (size_t) thiz);
	}
