...
public interface IInStream extends ISequentialInStream {
	/**
	 * Indicates, that the seek operation should be done from the beginning of
	 * the stream
	 */
	public static final int SEEK_SET = 0;

	/**
	 * Indicates, that the seek operation should be done from the current
	 * position (file pointer) of the stream. Positive values move file pointer
	 * forward.
	 */
	public static final int SEEK_CUR = 1;

	/**
	 * Indicates, that the seek operation should be done from the end of the
	 * stream. Positive offset values move file pointer over the end of the
	 * stream. For the read only streams it's equivalent reaching end of the
	 * stream while reading.
	 */
	 ...
	 *            <li> {@link #SEEK_SET} - <code>offset</code> is an absolute
	 *            offset to move to,<br><li>
	 *            {@link #SEEK_CUR} - <code>offset</code> is a relative offset
	 *            to the current position in stream,<br> <li> {@link #SEEK_END} -
	 *            <code>offset</code> is an offset from the end of the stream
	 *            <code>(offset < 0)</code>.
	 * 
