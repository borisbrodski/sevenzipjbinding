...
function createVitePluginAstroServer({
  ...
  return {
    ...
    async configureServer(viteServer) {
      ...
      const runnableAstroEnvironment = isRunnableDevEnvironment(astroEnvironment) ? astroEnvironment : void 0;
      const contentConfigLoad = runnableAstroEnvironment ? kickOffContentConfigLoad({
        settings,
        fs: nodeFs,
        logger,
        environment: runnableAstroEnvironment
      }) : Promise.resolve();
      const ssrHandlerPromise = runnableSsrEnvironment ? contentConfigLoad.then(() => createHandler(runnableSsrEnvironment)) : void 0;
      const prerenderHandlerPromise = runnablePrerenderEnvironment ? contentConfigLoad.then(() => createHandler(runnablePrerenderEnvironment)) : void 0;
      viteServer[devServerAppReadySymbol] = Promise.allSettled(
      ...
      async function handleUnhandledRejection(rejection) {
        ...
        const handlers = [];
        if (ssrHandlerPromise) handlers.push(await ssrHandlerPromise.catch(() => void 0));
        if (prerenderHandlerPromise)
          handlers.push(await prerenderHandlerPromise.catch(() => void 0));
